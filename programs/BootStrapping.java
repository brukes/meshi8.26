...
/***
    Data: 06/03/2011
    Creator: N. Vanetik
    This programs performs bootstrapping tests of distance distribution between pairs of atomOne types.
    The program receives following parameters:
        <atomOne type 1 (3 letters)> <atomOne type 2 (3 letters)> <distance (3-20)> <PDB database directory> <sample size> <number of samples>
***/
public class BootStrapping{
    ...
    /***
     Read file in PDB format and compute the number of atoms of types atom1 and atom2 at distance d
     ***/
	...
	if (pdbFile.exists() && pdbFile.getName().endsWith(PDB_EXTENSION))
	    {
		 ...
		 int residueNumber = 0;
                 ...
                 ArrayList Xcoordinate = new ArrayList();
                  ...
                  while ((line = pdbFileReader.readLine()) != null)
                        {
                            ...
                            if (line.trim().startsWith(PDB_ATOM_RECORD))
                            {
                                ...
                                // 3rd element is the atomOne name
                                if (st.hasMoreTokens())
                                ...
                                try {
                                    ...
                                    //System.out.println("read atomOne "+atomOne+" of residue "+
                  ...
                  pdbFileReader.close();
		  /*** now go over atomOne pairs and search for specified types and distance ***/
    ...
    /*** Find in the atomOne list pairs of residues of required types and at the required distance
                                             ...
                                             String type1, String type2, int requiredDistance, int pdbFileNumber)
    {
	 ...
	 double finalCount=0;
         ...
         for (int j=0; j<atoms.size(); j++)
             {
                 // find the first required atomOne of the pair
		 ...
		 if (!atom1.equals(type1) && !atom1.equals(type2))
		    ...
		    System.out.println("redudancy weight for file="+pdbFileNumber+" and residue="
                 ...
                 for (int k=j+1; k<atoms.size(); k++)    
                     {
                        // find the second required atomOne of the pair
			atom2 = ((AtomType)atoms.get(k)).toString();
		          ...
		          continue;
				     ...
				     if (DEBUG_LEVEL>=3)
                                      System.out.println("Found atomOne pair "+atom1+"-"+atom2+" at distance "+roundDistance);
