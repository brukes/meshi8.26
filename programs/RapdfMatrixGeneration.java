...
public class RapdfMatrixGeneration {
    ...
    /*** Update the entire matrix:
         ...
         (2.5) determine atomOne type with the help of AtomType enum
         (3) store atomOne coordinates in local array
         (4) keep track of residues
         (5) for each pair of atoms NOT IN THE SAME RESIDUE, toggle the counters by calling updateDistance ***/
    public void fillMatrix(String pdbDirectory)
     ...
     /*** Update the matrix pointwise, given a pair of atoms and a distance ***/
    public void updateDistance(AtomType a, AtomType b, double distance, int sequence, ArrayList dbRedundancy, int res1, int res2)
        {
          ...
          // check that neither atomOne is a hydrogen
                  ...
                  if (line.trim().startsWith(PDB_ATOM_RECORD))
                   {
                       //System.out.println("=========>  found ATOM record "+line);
                                ...
                                AtomType atom = AtomType.type(token, atomToken);
                                //System.out.println("=========>  found ATOM of type "+atomToken+" setResidue to type "+atomOne);
