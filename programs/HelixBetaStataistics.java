...
import meshi.energy.rg.CenterOfMass;
...
public class HelixBetaStataistics {
    public static void main(String[] args) throws IOException {
        ...
        while ((fileName = reader.readLine()) != null) {
            ...
            for (Atom atom1 : protein.atoms()) {
                ...
                if (atom1.residue().secondaryStructure() != SecondaryStructure.HELIX) continue;
                for (Atom atom2 : protein.atoms()) {
                    if ((!atom2.type().backboneH()) && (!atom2.type().backboneO())) continue;
                    if (atom2.residue().secondaryStructure() == SecondaryStructure.HELIX) continue;
                    if ((atom2.residue().secondaryStructure() == SecondaryStructure.COIL) && (Math.abs(atom1.residue().number() - atom2.residue().number()) <= 15))
                        continue;
                    if ((atom2.residue().secondaryStructure() == SecondaryStructure.SHEET) && (Math.abs(atom1.residue().number() - atom2.residue().number()) <= 4))
                        continue;
                        ...
                        if (atomA1 == null) {
                            ...
                            if (residueA1 != residueB1 &&
                                ...
                                System.out.println("\n_______________________________________\n" +
                                        protein.name() + "\n" +
                                        atomA1 + " " + atomA1.residue().secondaryStructure() + "\n" +
                                        atomA2 + " " + atomA2.residue().secondaryStructure() + "\n" +
                                        atomB1 + " " + atomB1.residue().secondaryStructure() + "\n" +
                                        atomB2 + " " + atomB2.residue().secondaryStructure() + "\n" +
                                        dA + "   \t\t    " + dB +
                                        "\n-------------------------------------------------------");
                            }
                        }
    ...
    public static boolean onTheSameSegment(Chain chain, Residue residue1, Residue residue2) {
        ...
        SecondaryStructure ss = residue1.secondaryStructure();
        int size = chain.size();
        int n1 = residue1.number();
        int n2 = residue2.number();
        for (int i = n1 + 1; (i < size) && (i <= n2); i++) {
            if (chain.get(i).secondaryStructure() != ss) return false;
        }
        return true;
    }
