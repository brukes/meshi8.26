 ...
 * <li> During most of an energy based simulation (e.g. MD, minimization, MC etc.) the atomOne
 * movements are rather slow, and the distance matrix changes very little between
 * consecutive simulation steps.<br>
 * This implies that most of the atomOne pairs that are "infinitely" distant in a given
 ...
 * The algorithm guaranties that if a pair of atoms have a distance within this range
 * it is included in the nonbonded list.</li>
 * <li> rMax < distance <= rMax + buffer                                                               <br>
 * Some of the atomOne pairs with a distance within this range are included in the nonbonded list.   </li>
 * <li> rMax < distance                                                                                <br>
 * These atomOne pairs are not included in the nonbonded list</li>
 * <p/>
 * The number of atomOne-pairs with inter-atomic distances in the first two regions is O(n). These
 * atomOne-pairs are stored in a list called the non-bonded-list. The only O(n^2) task is to test for
 * each of the O(n^2) atomOne-pairs currently in the third region whether it moved to the first two. This
...
//        System.out.println("xxxxxxxxxxx "+rMaxPlusBuffer+" "+new FreeDistance(molecularSystem.get(828).atom,molecularSystem.get(16).atom));
        ...
        for (DistanceList row : this) {
            if (!row.atomOneNowhere ) {
                //Ofer 18.4.15 added the &&_statement in the if above
                //This is because atomOneNowhere is false and atomOne.status().nowhere() is true
                //This happens after a certain number of proteins for some reason

                //for debug please remove
                //System.out.println("atomOne status : "+row.atomOne.status());
                //System.out.println("atomOne status.nowhere : "+row.atomOne.status().nowhere());


                GridCell gridCell = grid.getCell(row.atomOne);
                /// debug
                if (row.atomOneNumber == 828) {
//                    if (!gridCell.isEmpty()) {
//                        System.out.print("xxxxxxxyyyy ");
//                        for (AtomCore atomCore : gridCell) System.out.print(" " + atomCore.number);
//                        System.out.println();
//                    }
                }
                 //// debug
