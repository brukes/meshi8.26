...
//!	System.out.println(best + " " + dis);	
        ...
        // Now creating the new atomOne list
        prepareRMS(best);
        Overlap.rmsPartial(co, co2, aux);
        for (c = 0; c < SEGMENT; c++) {
            ...
            out.add(new Atom("CA", residue, aaType.caType(), new Coordinates(co[0][c], co[1][c], co[2][c]), new Double(0),molecularSystem));
            // Adding C
            //                                    out.add(new Atom(co2[0][SEGMENT+c],co2[1][SEGMENT+c],co2[2][SEGMENT+c],
            //                                    "C",aaType,al.atomAt(c).residueNumber(),-1));
            out.add(new Atom("C", residue, aaType.cType(), new Coordinates(co2[0][SEGMENT + c], co2[1][SEGMENT + c], co2[2][SEGMENT + c]), new Double(0),molecularSystem));
            // Adding N
            //                                    out.add(new Atom(co2[0][2*SEGMENT+c],co2[1][2*SEGMENT+c],co2[2][2*SEGMENT+c],
            //                        	         "N",aaType,al.atomAt(c).residueNumber(),-1));
            out.add(new Atom("N", residue, aaType.nType(), new Coordinates(co2[0][2 * SEGMENT + c], co2[1][2 * SEGMENT + c], co2[2][2 * SEGMENT + c]), new Double(0),molecularSystem));
            // Adding CB
            if (!aaType.equals(ResidueType.GLY)) {
                ...
                out.add(new Atom("CB", residue, aaType.cbType(), new Coordinates(co2[0][3 * SEGMENT + c], co2[1][3 * SEGMENT + c], co2[2][3 * SEGMENT + c]), new Double(0),molecularSystem));
            }
            // Adding O
            //                                   out.add(new Atom(co2[0][4*SEGMENT+c],co2[1][4*SEGMENT+c],co2[2][4*SEGMENT+c],
            //                                   "O",aaType,al.atomAt(c).residueNumber(),-1));
            out.add(new Atom("O", residue, aaType.oType(), new Coordinates(co2[0][4 * SEGMENT + c], co2[1][4 * SEGMENT + c], co2[2][4 * SEGMENT + c]), new Double(0),molecularSystem));
            // Adding H
            if (!aaType.equals("PRO")) {
                ...
                out.add(new Atom("H", residue, aaType.hType(), new Coordinates(co2[0][5 * SEGMENT + c], co2[1][5 * SEGMENT + c], co2[2][5 * SEGMENT + c]), new Double(0),molecularSystem));
            }
        }
        return out;
    }
