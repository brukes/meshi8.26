...
public class PutHpos {
    ...
    public static PutHposLog pos(Atom hydrogenAtom,
                                 ...
                                 AngleParametersList angleParametersList) throws NotEnoughBoundAtomsException {
        /* integer ih
                     logical success
                     character*3 name
                     integer namel
        ...
        if (iHydrogen.bonded().size() != 1)
            throw new RuntimeException("Weird number of atoms bonded to " + iHydrogen + " " + iHydrogen.bonded().size());
        Atom jNeighbor = iHydrogen.bonded().get(0);
        if (!iHydrogen.nowhere())
            throw new RuntimeException("Hydrogen atomOne already has coordinates.\n" + iHydrogen + "\n" + iHydrogen.core);
        if (jNeighbor.type().isHydrogen())
            throw new RuntimeException("Weird hydrogen " + jNeighbor + " bound to " + iHydrogen);
        if (jNeighbor.nowhere())
            throw new RuntimeException("Cannot assign coordinates to a hydrogen atomOne if it is bound to" +
                    " an atomOne of unknown position.");
        ...
        if (jNeighbor.bonded().size() <= 2) {
            throw new NotEnoughBoundAtomsException("An error while trying to assign coordinates to " +
                    iHydrogen + "\n" +
                    "The neighbor heavy atomOne " + jNeighbor + "\n" +
                    "has only " + jNeighbor.bonded().size() +
                    " boundAtoms\n" +
                    "Hydrogen neighbor with less than 3 bonds is not supported yet\n" +
                    "This needs to be changed in order to support all-atomOne-model");
        ...
        if (jNeighbor.bonded().size() == 3) {
            ...
            if (nNowhere == 3) throw new RuntimeException("This is really weird 4"); //Cannot assign coordinates
            if (nNowhere == 2) { // The hydrogen position is not uniquely defined.
                ...
                c as compared to the j atomOne coordinateis a linear combination
...
//			iHydrogen.setTemperatureFactor(jNeighbor.temperatureFactor());
              ...
              c in the present force field (extended atomOne model) but does exist
              c for all atomOne case is
