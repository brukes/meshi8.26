...
public class MolecularSystem extends ArrayList<AtomCore> {
    ...
    public void createDistanceMatrix(String message,DistanceMatrix.DistanceMatrixType distanceMatrixType) {
        if (message != null) Utils.println("Creating new DistanceMatrix because: " + message);
        ...
        terminator = new Terminator();
                  switch (distanceMatrixType) {
                    ...
                    case LONG_DISTANCES: distanceMatrix = new DistanceMatrix(this,distanceMatrixType);
                                         break;
                    default:throw new RuntimeException("do something");
                }
    }
    ...
    protected AtomCore createAtomCore(Atom atom, AtomType type, AtomStatus status, double x, double y, double z) {
        if (currentTotalEnergy != null) throw new RuntimeException("Cannot create more atoms in this MolecularSystem an energy function already uses it.");
        if (distanceMatrix != null) throw new RuntimeException("Cannot create more atoms in this MolecularSystem a distance matrix already uses it.");
        AtomCore newAtomCore = new AtomCore(atom, type, status, size(), x, y, z);
        add(newAtomCore);
        terminator.kill("An atomOne was added.");
        return newAtomCore;
    }
