...
public class Chain extends ResidueList {
    ...
    public Chain(ResidueList residueList, Protein protein) {
        this(residueList.get(0).chain(),protein);
        int residueNumber = 0;
        for (int iResidue = 0; iResidue < residueList.size(); iResidue++) {
            ...
            while (residueNumber < residue.ID().number())  {
                add(new Residue(new ResidueIdentifier(name,residueNumber)));
                residueNumber++;
            }
...
//            }
     ...
     * determines the molecular model.
     */
    public Chain(MeshiSequence sequence, ResidueCreator creator, String chainName, Protein protein) {
    ...
    private static AtomList getCaAtomList(char c, Coordinates coordinates,MolecularSystem molecularSystem) {
        ...
        Atom ca = new Atom("CA", null, type, coordinates, new Double(0),molecularSystem);
