...
public class ResidueList extends ArrayList<Residue> {
    ...
    public AtomList atoms() {
        if (size() < 1) throw new RuntimeException("Cannot create an atomOne list from an empty residue list.");
        return new AtomList(this,get(0).atoms().molecularSystem);
    }
    ...
    public void print(MeshiWriter writer) {
            for (Residue r : this)
               r.atoms().somewhere().print(writer);
        }
    ...
    public void freeze() {
        for (Residue residue : this) {
            residue.atoms().freeze("In ResidueList");
        }
    }
    ...
    /**
     * A secure defrost  method. Does not allow the simulation to continue after atoms have melted.
     */
    public void defrost() {
        for (Residue residue : this) {
            residue.atoms().defrost();
