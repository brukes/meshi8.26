...
// 	    e = storage.get(i);
    ...
    protected OptimizerStatus kickStart() throws OptimizerException, UpdateableException, EvaluationException {
        ...
        try {
            ...
            Utils.println("**************************************\n" + "Trying to run steepest decent with reduced bond.");
            BondEnergy bondEnergy = (BondEnergy) energy.getEnergyTerm(new BondEnergy());
            if (bondEnergy == null) throw ex;
            double weight = bondEnergy.weight();
            bondEnergy.setWeight(weight * 0.2);
            steepestDecent.run();
            Utils.println("**************************************\n" + "Restoring  bond weight.");
            bondEnergy.setWeight(weight);
            os = steepestDecent.run();
        }
        numKickStarts++;
