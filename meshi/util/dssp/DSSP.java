...
public class DSSP {
    ...
    */
    ...
    public String getRelativeACC() {
        ...
        for (int i = 0; i < resNum.length; i++)
            if (resNum[i] > -999) {
                //System.out.println("xxxxx "+i+"  "+aa[i]+" "+ResidueType.type(aa[i])+" "+ResidueType.type(aa[i]).ordinal())  ;
                if ((aa[i] == 'a') || (aa[i] == 'b') || (aa[i] == 'c') || (aa[i] == 'd'))
                    aa[i] = 'C';// Cysteins involved in disulphide bonds
                if ((ResidueType.type(aa[i]).ordinal() > 19) || (ResidueType.type(aa[i]).ordinal() < 0))
                    throw new RuntimeException("weird residue " + aa[i] + " type number " + ResidueType.type(aa[i]).ordinal());
    ...
    private static   String getDsspFileName(String fileName) {
    ...
    public SequenceList sequenceList() {
        ...
        MeshiSequence aaSeq = new ResidueSequence(getAA(), "aa sequence of " + fileName);
        seqList.add(aaSeq);
        MeshiSequence ssSeq = new SecondaryStructureSequence(getSSInOneLineConventional(), "ss sequence of " + fileName);
        seqList.add(ssSeq);
        MeshiSequence accSeq = new AccesibilitySequence(getRelativeACC(), "accesibility sequence of " + fileName);
        seqList.add(accSeq);
        seqList.add(accSeq);
        return seqList;
    }
