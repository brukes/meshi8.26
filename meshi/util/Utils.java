...
public class Utils implements MeshiPotential, KeyWords {
    ...
    public static ResidueAlignment alignProteinsByAlignmentFile(CommandList commands, Key key,
        ...
        return new ResidueAlignment(chain0, chain1, sequenceList, new IdentityMatrix());
    }
    ...
    public static void RmsGdtEnergy(Object output,
                                    Protein model, Protein reference,
                                    TotalEnergy totalEnergy, String tag,
                                    boolean headerFlag) throws EvaluationException,AlignmentException {
        ResidueAlignment modelRefeenceAlignment = null;
        int referenceSize = -1;
        if (reference != null) {
            modelRefeenceAlignment = new ResidueAlignment(reference.chain(), reference.name(), model.chain(), model.name(), ResidueAlignmentMethod.BY_RESIDUE_NUMBER);
            referenceSize = reference.atoms().CAFilter().size();
        }
        if (output instanceof StringList)
            RmsGdtEnergy((StringList) output, modelRefeenceAlignment, totalEnergy, tag, referenceSize, headerFlag);
        else if (output instanceof MeshiWriter)
            RmsGdtEnergy((MeshiWriter) output, modelRefeenceAlignment, totalEnergy, tag, referenceSize, headerFlag);
        else throw new RuntimeException("Wrong first parameter for RmsGdtEnergy " + output);
    }
    ...
    /*public static boolean addAtoms(Protein model,
            ...
            for (Chain chain :model.chains()){
                ...
                for (Residue residue : chain) {
                    ...
                    if ((prev != null) && (residue.number() != prev.number() + 1))
                        throw new RuntimeException("Cannot add atoms. Missing residue(s) between " + prev + " and " + residue);
                    prev = residue;
                }
            }
        }
        ...
        while (someAtomsAreNowhere(model)){
            DistanceMatrix dm = model.atoms().molecularSystem().getDistanceMatrix();
            for (Residue residue: model.residues()) {
                if (!residue.dummy()) {
					*//*for(Atom atomOne : residue.atoms().frozenAtoms()){
						if(atomOne.name().equals("CG")){
							atomOne.defrost();
						}
					}
					...
					*//*
                    ...
                    while (illegal(torsionList,param).size()>0) {
                        residue.atoms().defrostedAtoms().setNowhere();
  ...
  public static boolean addAtoms(Protein model,
      ...
      if (! allowMissingResidues) {
        ...
        while (someAtomsAreNowhere(model)){
            ...
            while (illegalTorsions.size()>0) {
                ...
                System.out.println("Trying to fix "+torsion);
                Atom distantAtom = torsion.distantAtom();
                assignRandomCoordinates(distantAtom);
                dm.reset();
                if (counter++ > 100)  {
                    torsion.atoms().setNowhere();
                    torsion.atom1.residue().atoms().backbone().sideChains().setNowhere();
                }
                    ...
                    if (fixFlag) {
                        for (Atom atom: model.atoms())
                            if (atom.energy() > 10000)
                                atom.residue().atoms().sideChains().setNowhere();
                    }
                }
                catch (EvaluationException ex) {
                    dm = model.atoms().molecularSystem().getDistanceMatrix();
    ...
    private static TorsionList illegal(TorsionList torsionList, ParametersList parametersList) {
        ...
        for(Torsion torsion : torsionList){
			...
			/*for(Atom atomOne : torsion.atoms()){
				System.out.println(atomOne.frozen()+"   " + atomOne.name+" !!!!!!!!!!!!!!!!!");
			}
...
//                System.out.println(parameters);
        ...
        System.out.println(out.size()+" illegal torsions");
        return out;
    ...
    private static boolean createAndCheckIfElementIsLegal(Torsion baseElement,
        ...
        if ((element.getIsomer() == PlaneParameters.Isomer.CIS) & (element.getTorsion().cosTorsion()< 0)) {
            ...
            System.out.println(element+" "+element.getIsomer()+"   "+element.getTorsion().cosTorsion());
            return false;
        }
...
/*
  ...
  */
    ...
    public static String getProteinNameFromPdbFileName(String fullFileName) {
        ...
        StringTokenizer words = new StringTokenizer(fileName, ".");
...
//	          }
    ...
    /*public static boolean assignRandomCoordinates(Residue residue, CommandList commands, int maxMissingAtoms) {
        ...
        for (Iterator atoms = residue.atoms().iterator(); atoms.hasNext();) {
            Atom atomOne = (Atom) atoms.next();
            if (atomOne.nowhere()) nowhereAtoms.add(atomOne);
            if (atomOne.nowhere() & (!atomOne.type().isHydrogen())) nowhereHeavyAtoms.add(atomOne);
        }
        if (nowhereAtoms.size() == 0) {
            println("assignRandomCaCoordinates to " + residue + " nothing to do");
            return true;
        }
        if (nowhereHeavyAtoms.size() > maxMissingAtoms) {
            println("assignRandomCaCoordinates to " + residue + " missig atoms " + nowhereHeavyAtoms.size() + " maximum " + maxMissingAtoms + " do nothing");
            return false;
        }
        for (Atom atomOne : nowhereAtoms) {
                Atom neighbor = getCovalentNeighbor(atomOne);
                if (neighbor == null) {
                    println("\n\n Problem in Util.assignRandomCoordinates(Residue residue, CommandList commands, int maxMissingAtoms)");
                    println("no CovalentNeighbor to " + atomOne);
                    residue.atoms().print();
                    throw new RuntimeException("This is weird");
                }
            neighbor = null;
            for (Atom tmp : atomOne.bonded())
                if (!tmp.nowhere()) neighbor = tmp;
            if (neighbor != null) {
                atomOne.setXYZ(new Coordinates(new Coordinates(neighbor), 1.5));
                println("assignRandomCaCoordinates to " + atomOne);
                return true;
            }
        }
        return false;
    ...
    */
        ...
        for (Residue residue : chain) {
            if (first) first = false;
                ...
                if (!residue.dummy()) {
                    ...
                    prev = new Coordinates(residue.ca());
                    if (!residue.secondaryStructure().equals(SecondaryStructure.HELIX) && residue.position() % boxLength == 0) {
                        direction *= -1;
                        double temp = bias[0];
                        bias[0] = bias[1];
                        bias[1] = bias[2];
        ...
        for (Residue residue : chain) {
            if (first) first = false;
                ...
                if (!residue.dummy()) {
                    ...
                    if (changeDirection && residue.secondaryStructure().equals(SecondaryStructure.COIL)) {
        ...
        for (Residue residue : chain) {
            if (first) first = false;
                ...
                if (!residue.dummy()) {
                    ...
                    println("dis: " + dis);
                    if (!residue.secondaryStructure().equals(SecondaryStructure.HELIX) && dis > sphereRadius) {
                        direction *= -1;
                        double temp = bias[0];
                        bias[0] = bias[1];
                        bias[1] = bias[2];
        ...
        for (Residue residue : chain) {
            if (first) first = false;
                ...
                if (!residue.dummy()) {
                    ...
                    } while (!residue.secondaryStructure().equals(SecondaryStructure.HELIX) && dis > sphereRadius);
                }
            }
        }
    }
    ...
    /**
     * May be used only by getCovalentNeighbor(Atom atomOne)
     */
...
//	for (Iterator atoms = atomList.iterator(); atoms.hasNext();) {
//	    Atom atomOne = (Atom) atoms.next();
//	    atomOne.setTemperatureFactor(99.9*((atomOne.energy()-min)/range));
//	}
//    }
    ...
    public static boolean AssignDSSP(Protein protein, String dsspFileName) {
        ...
        MeshiSequence residueSequence = OneDimView.getResidueSequence();
        ...
        MeshiSequence fullSequence = protein.sequence();
...
//                        double relativeAccesibility = Double.valueOf(OneDcell2.comment);
    ...
    public static AtomList duplicateInAnewMolecularSystem(AtomList atomList, MolecularSystem newMolecularSystem) {
        ...
        for (Atom atom : atomList) {
            out.add(new Atom(new PdbLine(atom.toString()),newMolecularSystem));
        }
        return out;
    }
    ...
    public static String pdb2Fasta(File in) throws IOException{
        ...
        System.out.println(">"+protein.name()+" ; "+protein.chain().toString());
        writer.println(">"+protein.name()+" ; "+protein.chain().toString());
        writer.close();
        return fastaName;
    }
}
