...
public class GDTcalculator {
    ...
    public static void init(AtomList al1, AtomList al2) {
        if (al1.size() != al2.size()) throw new RuntimeException("The atomOne lists need to have the same size");
        al1 = al1.CAFilter();
        al2 = al2.CAFilter();
        if (al1.size() != al2.size()) throw new RuntimeException("The two CA atomOne lists need to have the same size");
