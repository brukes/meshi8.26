 ...
 * 3) In the second mode, the protein instance must contain ALL the atomOne including hydrogens.
...
public final class Scmod extends MeshiProgram implements KeyWords, MeshiPotential {
    ...
    public static void scmod(CommandList commands, Protein protein, int maxIter, double excludedVolumeWeight) throws UpdateableException, EvaluationException{
    ...
    public static void scmod(CommandList commands, Protein protein, int maxIter,
                             ...
                             boolean conservative, Random random) throws UpdateableException, EvaluationException{
        int ind;
        int pred;
        int numOfRotamers;
        double[] rotamer;
        ...
        for (int residueIndex = 0; residueIndex < protein.residues().size(); residueIndex++) {
            if (!protein.residues().get(residueIndex).dummy())
                if (protein.residues().get(residueIndex).ca().frozen())
                    protein.residues().get(residueIndex).atoms().freeze("In Scmod");
        ...
        if (totalEnergy == null) {
            ...
            protein.molecularSystem.createDistanceMatrix();
            ...
            for (int residueTypeIndex = 0; residueTypeIndex < seder.length; residueTypeIndex++) {
                ...
                for (int residueIndex = 0; residueIndex < pp.length; residueIndex++) {
                    ...
                    if ((pp[residueIndex] != null) && (pp[residueIndex][2] == residueType) &&
                        ...
                        minscore = 1e15;
                        pred = -1;
                        if (((stage > 0) & (!conservative)) & (energyTerm != null)) {
                        ...
                        for (ind = 0; ind < numOfRotamers; ind++) {
                            ...
                            badPoints = calcVariousEnergies(residueIndex, probrot, stage, protein, energy,
                                                            protein.atoms().molecularSystem().getDistanceMatrix(),
                                                            energyTerm, parametersList);
    ...
    protected static double[] calcVariousEnergies(int resnum, double prob, int stage, Protein protein,
        ...
        if (stage == 0) {
            ...
            try {
                ...
                throw new RuntimeException("calcVariousEnergies failed due to " + ae);
            }
            if (energyTerm != null) result[5] = energyTerm.getNonWeightedHBenergy();
            else result[5] = 0;
        }

        // reseting the relevent atomOne energies
        AtomList al = protein.residue(resnum).atoms();
        Atom atom1, atom2;
        for (Atom atom : al)
            atom.resetEnergy();
        // Calculating the EV isOn the relevent atoms
        for (DistanceList distanceRow : distanceMatrix.nonBondedList()) {
