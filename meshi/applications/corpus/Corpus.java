...
public class Corpus implements CompositeTorsionsDefinitions, MeshiPotential, KeyWords {
    ...
    protected double[][][] coors = new double[0][][]; // Indexing-[residue index][atomOne: N=0 ; Ca=1 ; C=2][coordinates x=0;y=1;z=2]
...
// Auxilary method
    ...
    private static void extractCoordinatesOfRes(Residue res, double[][] co) {
        ...
        co[0][0] = amideN.x();    //res.atoms().findAtomInList("N" , res.number).x();
        co[0][1] = amideN.y();    //res.atoms().findAtomInList("N" , res.number).y();
        co[0][2] = amideN.z();    //res.atoms().findAtomInList("N" , res.number).z();
        co[1][0] = ca.x();        //res.atoms().findAtomInList("CA" , res.number).x();
        co[1][1] = ca.y();        //res.atoms().findAtomInList("CA" , res.number).y();
        co[1][2] = ca.z();        //res.atoms().findAtomInList("CA" , res.number).z();
        co[2][0] = carbonylC.x(); //res.atoms().findAtomInList("C" , res.number).x();
        co[2][1] = carbonylC.y(); //res.atoms().findAtomInList("C" , res.number).y();
        co[2][2] = carbonylC.z(); //res.atoms().findAtomInList("C" , res.number).z();
    }
...
// ************************
    ...
    // Take the coordinates of mutateThis and the flanks backbone, and add 20 to all their atoms.
    ...
    private void specialMutate(Protein aux, int mutateThis, int mutateTo, double[][] pp, DunbrackLib lib) {
        Atom atom;
        atom = aux.residue(mutateThis - 1).amideN();                          //aux.residue(mutateThis-1).atoms().findAtomInList("N" , mutateThis-1);
        aux.residue(mutateTo - 1).amideN().setXYZ(atom.x(), atom.y(), atom.z());//aux.residue(mutateTo-1).atoms().findAtomInList("N",mutateTo-1).setXYZ(atomOne.x(),atomOne.y(),atomOne.z());
        atom.setXYZ(atom.x() + 20, atom.y() + 20, atom.z() + 20);

        atom = aux.residue(mutateThis - 1).ca();                          //aux.residue(mutateThis-1).atoms().findAtomInList("CA" , mutateThis-1);
        aux.residue(mutateTo - 1).ca().setXYZ(atom.x(), atom.y(), atom.z());//aux.residue(mutateTo-1).atoms().findAtomInList("CA" , mutateTo-1).setXYZ(atomOne.x(),atomOne.y(),atomOne.z());
        atom.setXYZ(atom.x() + 20, atom.y() + 20, atom.z() + 20);

        atom = aux.residue(mutateThis - 1).carbonylC();                           //aux.residue(mutateThis-1).atoms().findAtomInList("C" , mutateThis-1);
        aux.residue(mutateTo - 1).carbonylC().setXYZ(atom.x(), atom.y(), atom.z()); //aux.residue(mutateTo-1).atoms().findAtomInList("C",mutateTo-1).setXYZ(atomOne.x(),atomOne.y(),atomOne.z(
        atom.setXYZ(atom.x() + 20, atom.y() + 20, atom.z() + 20);
