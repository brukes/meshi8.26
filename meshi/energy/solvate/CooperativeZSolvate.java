                ...
                switch (typeOfSolvateDis) {
                    ...
                    case TWO_POLARS:
                        ...
                        if (sigmaValuesBP.sigmHB > 0.0) { // The HB related derivatives
                            ...
                            solvateTerm.solvateHB.findBondByPolars(atom1.atom, atom2.atom).applyForcesToAtoms(solvFactor);
                        }
                        break;
                }
...
//               */
        ...
        for (int i = 0 ; i<molecularSystem.size() ; i++) {
            if ( molecularSystem.get(i).atomOne.nowhere()) continue;
            atomOne = molecularSystem.get(i).atomOne;
            if (! atomOne.frozen()) {
                type = atomOne.type();
                ...
                factor    = diff*weight;
                if (factor != 0) {
                    atomOne.addToFx(-solvateTerm.forceX[i]*factor);
                    atomOne.addToFy(-solvateTerm.forceY[i]*factor);
                    atomOne.addToFz(-solvateTerm.forceZ[i]*factor);
                }
            }
