 ...
 * Value i,j in the matrix, is the relevent sigmoid property of atomOne type j isOn atomOne type i. For example, the sixth value in the
 ...
 * calculates the HB index.
 * <p/>
 * 11) SolvateMESHI2Tsai.dat - In MESHI there are 190 defined atomOne types. In the solvate energy, we sometime use
 * a reduce representation of 14 atomOne types as defined in Tsai et. al. 99'. The format of this file is:
 * {MESHI atomOne type (string)} {Tsai type number (int)}
 * {MESHI atomOne type (string)} {Tsai type number (int)}
 * {MESHI atomOne type (string)} {Tsai type number (int)}
 * ...
 * {MESHI atomOne type (string)} {Tsai type number (int)}
 ...
 * A spline is created for each of the of the 190 atomOne types, but it is a zero spline for those that are
...
public class SolvateParametersList implements Parameters {
    ...
    public final static int N_TSAI = 14; // The number of atomOne types used in Tsai 99'. Any Hydrogen is type 14.
    public final int[] atomicTypeConverter; // From MESHI atomOne types to Tsai 99'.
    ...
    public SolvateParametersList(String[] parameterFiles) {
        ...
        // Converting the 190 MESHI atomOne types to the 14 mentioned in Tsai 99'
        Utils.println("Reading solvation parameter file: " + parameterFiles[10]);
        try {
            // first pass isOn the file - to find the maximal atomOne type
        ...
        Utils.println("Reading solvation parameter file: " + parameterFiles[12]);
        try {
        ...
        Utils.println("Reading solvation parameter file: " + parameterFiles[13]);
        try {
