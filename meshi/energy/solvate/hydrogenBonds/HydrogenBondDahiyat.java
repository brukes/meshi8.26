 ...
 * 1) The hydrogen atomOne in the HB is not known explicitly (NoH). In this case a2 and a3 are the polar atoms,
 * and a1 and a4 are the heavy atoms to which they are attached (base atoms).
 * 2) The hydrogen atomOne in the HB is known explicitly (WithH). Either a2 or a3 must be a hydrogen. The other atomOne
 * COVELANTLY bonded to the hydrogen must be the polar atomOne.
 * <p/>
 ...
 * <p/>
 * Note:
 * 1) The constructor assume that the atomOne list it get as a parameter has the donor and acceptor in the first
 * two places (see documentation of mother class). The third place in the list is the atomOne that is covalently bonded
 * to the first atomOne. The forth place in the list is the atomOne that is covalently bonded to the second atomOne.
...
public class HydrogenBondDahiyat extends AbstractHydrogenBond implements DahiyatImplementationConstants {
    ...
    // The 4 atoms in atomOne list a referenced by these fields, according to the numbering of the
    ...
    // These fields tell how to convert from the 1..4 atomOne numbering to the initial atomOne list numbering
    ...
    public void updateHBvalueAndDerivatives() {
        ...
        if (hbDistance.mode() == DistanceMode.INFINITE) {
            System.out.println("xxxxxxxxxxxxxxxxxxxxxx "+hbDistance);
        }
    ...
    private final void updateAngProduct1234() {
        if (hbDistance.mode() == DistanceMode.INFINITE) {
            System.out.println("xxxxxxxxxxxxxxxxxxxxxx1 "+hbDistance);
        }

        updateAng123();
        if (hbDistance.mode() == DistanceMode.INFINITE) {
            System.out.println("xxxxxxxxxxxxxxxxxxxxxx2 "+hbDistance);
        }
        updateAng234();
    ...
    private final void updateAng234() {
        if (hbDistance.mode() == DistanceMode.INFINITE) {
            System.out.println("xxxxxxxxxxxxxxxxxxxxxx3 "+hbDistance);
        }
