...
import meshi.energy.solvate.CooperativeZSolvate;
 ...
 * all the atoms participating in the bond. The only condition isOn the list is
...
public abstract class AbstractHydrogenBond {
    ...
    /**
     * derivatives[i][0] is the derivative of the HB value relative to the X coordinate of atomOne i.
     * derivatives[i][1] is the derivative of the HB value relative to the Y coordinate of atomOne i.
     * derivatives[i][2] is the derivative of the HB value relative to the Z coordinate of atomOne i.
     */
...
//                        "\n"+atomOne);
    ...
    public void applyForcesToAtomsSolvCoop(double factor, CooperativeZSolvate coopZsolvTerm) {
        Atom atom;
        double std, factorCoopZsolv;
        for (int c = 0; c < atomList.size(); c++) {
            atom = atomList.atomAt(c);
            if (!atom.frozen()) {
                /*         int a = 0;
                       ...
                       a = 4;
                */
                std = coopZsolvTerm.std[atom.type().ordinal()];
                if (std == 0) continue;
                factorCoopZsolv = 4 * coopZsolvTerm.e3 / std * coopZsolvTerm.weight();
                atom.addToFx(-factorCoopZsolv * factor * derivatives[c][0]);
                atom.addToFy(-factorCoopZsolv * factor * derivatives[c][1]);
                atom.addToFz(-factorCoopZsolv * factor * derivatives[c][2]);

                atom.addToFx(-coopZsolvTerm.factor * factor * derivatives[c][0]);
                atom.addToFy(-coopZsolvTerm.factor * factor * derivatives[c][1]);
                atom.addToFz(-coopZsolvTerm.factor * factor * derivatives[c][2]);
