...
/**
 * A super class with the most general treatment of a list of hydrogen bonds in an atomOne list.
 ...
 * "bondList" vector that stores all the hydrogen bond objects relevent to the atomOne list. Whenever
...
public abstract class AbstractHydrogenBondList implements Updateable {
    ...
    protected int[] lut;     // The look-up table (lut) converts the atomOne internal number (field of Atom) to its index in the atomOne list given to the constructor.
    protected AbstractHydrogenBond[][] lutHB; // lutHB[i] is an array of pointers to all the hydrogen bond objects where atomOne number i is either their donor or acceptor. It is null for non-polar atoms, or atoms not participating in a hydrogen bond.
    ...
    private void addNewBondsToList() {
        ...
        for (DistanceList newNonBondedRow : newNonBondedList) {
            ...
            if (atom1.type().isCarbon() || atom1.type().isHydrogen() ) continue; // We seek oxygen and nitrogen atoms.
