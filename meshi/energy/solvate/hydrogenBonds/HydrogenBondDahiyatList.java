...
public class HydrogenBondDahiyatList extends AbstractHydrogenBondList {
    ...
    /**
     * This array stores pointers to the base atoms of every non-hydrogen polar atomOne in the protein.
     * The indexation is through the atomOne number (field)
     */
     ...
     * For any polar atomOne (O,N or S in Cys) we calculate the base atomOne that participate in the definition of
     * the hydrogen bond angle. This base atomOne is the attached hydrogen (if present), or the heavy atomOne to which
     * the polar atomOne is attached (when the hydrogen is not present).
     */
    ...
    protected AbstractHydrogenBond createHBfromPolars(Atom atom1, Atom atom2) {
        ...
        int TsaiAtomicType1 = parameters.atomicTypeConverter[atom1.type().ordinal()]; // Converting from the 190 atomOne types to the 14 defined in Tsai 99'
        int TsaiAtomicType2 = parameters.atomicTypeConverter[atom2.type().ordinal()]; // Converting from the 190 atomOne types to the 14 defined in Tsai 99'
