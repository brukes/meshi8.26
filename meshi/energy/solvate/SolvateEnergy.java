 ...
 * <p/>
 * The class allows to give a different weight to the solvation energies of certain atomOne types in the final
 * summation described in Eq. 5. These atomOne types are side-chain polars, side-chain carbons, and backbone
 ...
 * CNC's and HBC's of its atomOne pairs (Eqs. 1 and 2, respectively). The partial derivatives of the CNC's and HBC's
 ...
 * 2) A pass isOn the atomOne list. Once we have the CNC and HBC of every atomOne in the protein, we can proceed to calculate the solvation energy
 * associated with every atomOne. In this implementation we combined the EI(CNC,HBC) evaluation (Eq. 3) of every atomOne and
 * the -log(spline(EI)) evaluation (Eq. 4) into a single step by using a 2D spline, i.e. spline2D(CNC,HBC). The 2D spline
 * is, of course, atomOne type specific. The derivatives of each atomOne solvate energy value with respect to the HBC and CNC
 * are also calculated.
 * 3) A second pass over the non-bonded list. Equiped with the The derivatives of the atomOne energies (with respect
...
public final class SolvateEnergy extends CooperativeEnergyTerm {
     ...
     * instances of the arrays. The lengths of these arrays is the length of the atomOne list.
     * The indexing to these arrays is by the index of the atomOne in the atomOne list.
     */
     ...
     **/
    /** Size of the atomOne list. **/
    /**
     * The instance of the parameter list object. *
     */
    private SolvateParametersList parameters;
    /**
     * The look-up table (lut) converts the atomOne internal number (field of Atom), which is the index of the array, to its
     * index in the atomOne list given to the constructor. *
     */
    private int[] lut;

    /**
     * Setting the general type for each atomOne in the atomOne list: (0) Carbon (1) Backbone polar, (2) Sidechain polar (3) Hydrogens
     * This is done to save time isOn type checking. The index to the array is the index of an atomOne in the atomOne list. *
     */
     ...
     * index of the atomOne type (i.e. 0-189, because we have 190 atomOne types in meshi currently. The number 190 is not hard coded) *
     */
     ...
     * the atomOne type (i.e. 0-189, because we have 190 atomOne types in meshi currently. The number 190 is not hard coded). The splines
    ...
    public SolvateEnergy(AtomList atomList,
        ...
        weightSCPolarSolvate = solvateInfo.scPolarInfo.weight();
        weightBBPolarSolvate = solvateInfo.bbPolarInfo.weight();
        weightSCCarbonSolvate = solvateInfo.scCarbonInfo.weight();
        ...
        // Determining the superType for each atomOne: (0) Hydrophobic, (1) Backbone polar, (2) Sidechain polar (3) Hydrogens
        for (c = 0; c < molecularSystemSize; c++) {
            atom = molecularSystem.get(c).atom;
            atomType = atom.type();
            if (atomType.isCarbon() || (atomType == AtomType.MSD))
                ...
                throw new RuntimeException("The following if's should have covered all cases. But due to my carelessness this atomOne was left out:\n" +
                        atom + "\n");
            stat = new Stat();
        }
    }  // Of constructor
    ...
    public void firstPassOverTheNonBondedList() {
        ...
        for (DistanceList distanceRow : nonBondedList) {
            AtomCore atom1 = distanceRow.atomOne;
            atomNumber1 = distanceRow.atomOneNumber;
            for (Distance distance : distanceRow) {
                sigmaValues = (SolvateDistanceAttribute) distance.getAttribute(SolvateDistanceAttribute.SOLVATE_ALL_ATOM_ATTRIBUTE);
                // In case there is not a "SolvateDistanceAttribute" in the distance, we create one.
                if (sigmaValues == null) {
                    sigmaValues = SolvateDistanceAttributeCreator.create(distance, parameters, solvateHB);
    ...
    public double calculatingTheSolvationEnergiesOfEachAtom(boolean updateAtoms, double W_SCPolarSolvate, double W_SCCarbonSolvate, double W_BBPolarSolvate) {
        ...
        // ***************************************************************************
        // A pass over the atomOne list. Calculating the solvation energies of each atomOne.
        // ***************************************************************************
        ...
        for (AtomCore atom : molecularSystem) {
            ...
            if (energy > 100000)
                System.out.println("xxxxxxxxxxxxxxxxxxxxxxxxx "+energy);
            if (updateAtoms)
        ...
        ((DoubleInfoElement) info.infoList().get(0)).setValue(energySCPolarTotal);
        ((DoubleInfoElement) info.infoList().get(1)).setValue(energySCCarbonTotal);
        ((DoubleInfoElement) info.infoList().get(2)).setValue(energyBBONtotal);
         return energy;
    }
                ...
                switch (type) {
                    ...
                    case TWO_POLARS:
                        ...
                        if (sigmaValuesBP.sigmHB > 0.0) { // The HB related derivatives
                            solvateHB.findBondByPolars(distance.atom1(), distance.atom2()).applyForcesToAtoms(
    ...
    public void assignForcesToAtoms() {
        int cc;
        Atom atom;
        // Finally, the appropriate forces are assigned for every atomOne.
        for (cc = 0; cc < molecularSystem.size(); cc++) {
            ...
            for (int jDirection = -1; jDirection < 3; jDirection += 2) {
                ...
                for (int i = 0; i < CNC.length; i++) {
                    if (Math.abs(cncSave[i]-CNC[i])> 0.01)  {
                        System.out.println("problematic atomOne "+molecularSystem.get(i));
