...
public class CooperativeZSummaPolarElement {
    ...
    double[] zArray = new double[NumOfClusters];
...
//proceed Z[i] and derivatives
        ...
        zEnergyDerivative[atomNumber] = 0;
        if (zEnergy[atomNumber] > 0) {
            for (int i = initialPickNumber; i < NumOfClusters; i++) {
                allButI = 1;
                for (int j = initialPickNumber; j < NumOfClusters; j++) {
                    if (i != j) allButI *= zPenalty[j];
                }
                zEnergyDerivative[atomNumber] += z_weight * zPenaltyDerivative[i] * allButI;
            }
        }
        ...
        for (int i = initialPickNumber; i < NumOfClusters; i++) {
            ...
            zArray[i] += (zScoredAtomEnergies[i] * expEnergy);
            summNotI = 0;
            for (int j = initialPickNumber; j < NumOfClusters; j++) {
                if (j != i)
            ...
            derivedExpOfZScoredAtomEnergies[i][atomNumber] = expEnergy / std[i][type] + zScoredAtomEnergies[i] * (expOfZScoredAtomEnergies[i] * summNotI / (summPlusEpsilon * summPlusEpsilon));
        }
