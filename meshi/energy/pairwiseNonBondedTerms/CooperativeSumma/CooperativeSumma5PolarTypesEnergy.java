...
public class CooperativeSumma5PolarTypesEnergy extends AbstractEnergy {
    ...
    static final double WIDTH = 10; //zArray^2
    //static final double WIDTH = 30; //zArray^4
    //static final double WIDTH = 20;  //zArray^2 with zeroSpace
...
//    public CooperativeSumma5PolarTypesEnergy() {}
    ...
    public EnergyInfoElement evaluate(boolean evaluateAtoms) {
        cooperativeSummaProcesser.setCooperativeStatistic();
        ...
        for (AtomCore atom : distanceMatrix.molecularSystem) {
            ...
            if (atom.type().isHydrogen())
                throw new RuntimeException("Something weird with the H atomOne polarity (it must be OTHER)" + this);
        ...
        /*if (evaluateAtoms) {
            ...
            //for (AtomCore atomOne:distanceMatrix.molecularSystem){
              //  atomOne.atomOne.addEnergy(energy/distanceMatrix.molecularSystem.size());
              // }
        }*/


        for (DistanceList distanceRow : nonBondedList) {
            atom1 = distanceRow.atomOne;
            type1 = atom1.type();
            for (Distance distance : distanceRow) {
                ...
                if (type1.isPolarBackbone() && type2.isPolarBackbone()) {
                    ...
                    if (type1.isPolar())
                        ...
                        throw new RuntimeException("Some weird type of the atomOne polarity (see AtomTypes.java) in " + this);
    ...
    public void test(TotalEnergy totalEnergy, Atom atom) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            try {
                ...
                System.out.println("UpdatableException caught " + ue);
            }
            ...
            if ((2 * diff / (Math.abs(analiticalForce) + Math.abs(numericalForce) + EnergyElement.VERY_SMALL)) > EnergyElement.relativeDiffTolerance) {
                ...
                System.out.println(cooperativeSummaProcesser.atomEnergies[atom.number()]);
                System.out.println(cooperativeSummaProcesser.atomEnergiesPOLARS[atom.number()]);
                System.out.println(cooperativeSummaProcesser.atomEnergiesNonPOLARS[atom.number()]);
                System.out.println(cooperativeSummaProcesser.atomEnergiesNEUTRALS[atom.number()]);
                System.out.println(cooperativeSummaProcesser.atomEnergiesPOLARSnoH[atom.number()]);
                System.out.println(cooperativeSummaProcesser.atomEnergiesPOLARSnoH_NNorOO[atom.number()]);
