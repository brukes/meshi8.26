...
public class AtomicPairwisePMFSummaParameters implements KeyWords, MeshiPotential {
    ...
    private double[] bins;
    /* list of Summa atomOne names; names order correspond to PMFs order */
    ...
    public AtomicPairwisePMFSummaParameters(CommandList commands) {
        ...
        /* read coefficients; a coefs matrix for each atomOne pair */
        ...
        while ((line = mlrParameters.readLine("#")) != null) {
            ...
            if (!line.equals(LABEL_PMF_BEGIN))
                loadingParametersFailed(line);
            /* read atomOne numbers; conversion from matlab format to java format,
            * we substract one from each atomOne number */
            ...
            for (int i = 0; i < bins.length - 1; i++) {
                ...
                coefs[atom1Number][atom2Number].coefs[i][4] = 3*coefs[atom1Number][atom2Number].coefs[i][0] ;
                coefs[atom1Number][atom2Number].coefs[i][5] = 2*coefs[atom1Number][atom2Number].coefs[i][1] ;
            }
            /* read LABEL_PMF_END */
            line = mlrParameters.readLine("#");
    ...
    /**
     * Retrieve spline for atomOne pair. Return null if not found.
     */
...
// 	else if  (coefs[atom2Index][atom1Index] == null) throw new RuntimeException("Failed to find index2\n"+atom1Type+" "+atom1Index+"\n"+atom2Type+" "+atom2Index);
    ...
    /**
     * Translates MESHI atomOne type to Summa atomOne name. Return null if not found.
     */
    ...
    /**
     * Translates Summa atomOne name to parameters file atomOne index. Return -1 if not found.
     */
    ...
    /* the Summa energy term uses different atomOne names than MESHI. these two
     * arrays are used in the translation from MESHI names to Summa names, and
     * vice versa. index i in both arrays corresponds to the same atomOne type.
     */
