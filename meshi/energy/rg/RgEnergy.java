...
public class RgEnergy extends AbstractEnergy {
    ...
    public RgEnergy(EnergyInfoElement info) {
        this(getDummyUpdatables(), new LogRadiusOfGyration[6],0,info);
        off();
    }
    public RgEnergy(Updateable[] updateables, LogRadiusOfGyration[] logRgArray, double logN, EnergyInfoElement info) {
        super(updateables, info);
        comment = "RG";
        this.logN = logN;
    ...
    public EnergyInfoElement evaluate() {
        ...
        double energy = 0;
       energy += rgInfo.N_RG_HSS.setValue(evaluateN_RGhSS(rgInfo.RGlogaritmicWeight));
       energy += rgInfo.E_HSS_HCOIL.setValue(evaluateRGratio(centerOfMass, logNonPolarSsRG, logNonPolarCoilRG, RGhSS_RGhCoilCentered, testAtom, rgInfo.RGratioWeight, rgInfo.HSS_HCOIL));
       energy += rgInfo.E_HSS_BSS.setValue(evaluateRGratio(centerOfMass, logNonPolarSsRG, logBackboneSsRG, RGhSS_RGbSScentered, testAtom, rgInfo.RGratioWeight, rgInfo.HSS_BSS));
       energy += rgInfo.E_HSS_BCOIL.setValue(evaluateRGratio(centerOfMass, logNonPolarSsRG, logBackboneCoilRG, RGhSS_RGbCoilCentered, testAtom, rgInfo.RGratioWeight, rgInfo.HSS_BCOIL));
       energy += rgInfo.E_HSS_PSS.setValue(evaluateRGratio(centerOfMass, logNonPolarSsRG, logPolarSsRG, RGhSS_RGcSScentered, testAtom, rgInfo.RGratioWeight, rgInfo.HSS_PSS));
       energy += rgInfo.E_HSS_PCOIL.setValue(evaluateRGratio(centerOfMass, logNonPolarSsRG, logPolarCoilRG, RGhSS_RGcCoilCentered, testAtom, rgInfo.RGratioWeight, rgInfo.HSS_PCOIL));
       energy += rgInfo.HSS.setValue(evaluateLinearRG(centerOfMass, nonPolarRG, rgInfo.RGnonPolarWeight));
        energy += rgInfo.BSS.setValue(evaluateLinearRG(backboneCenterOfMass, backboneRG, rgInfo.RGbackboneWeight));
        energy += rgInfo.PSS.setValue(evaluateLinearRG(centerOfMass, polarRG, rgInfo.RGpolarWeight));
    ...
    private double evaluateRGratio(CenterOfMass centerOfMass, LogRadiusOfGyration logRG1, LogRadiusOfGyration logRG2, RgParameters parameters,
                                   Atom testAtom, double myWeight, DoubleInfoElement infoElement) {
        double energy = 0;
...
public void scaleWeight(double factor) {
    ...
    private void testLinearRG(CenterOfMass centerOfMass, RadiusOfGyration radiusOfGyration, double factor, Atom atom) {
              System.out.println(" testLinearRG(CenterOfMass centerOfMass, RadiusOfGyration radiusOfGyration, double factor, Atom atomOne) ");
            ...
            rg = radiusOfGyration.radiusOfGyration();
             ...
             // radiusOfGyration.test(atomOne);
             // centerOfMass.test(atomOne);
        }
