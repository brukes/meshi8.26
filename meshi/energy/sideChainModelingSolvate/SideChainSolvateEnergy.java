...
public final class SideChainSolvateEnergy extends CooperativeEnergyTerm {
     ...
     */
    private int[] lut;     // The look-up table (lut) converts the atomOne internal number (field of Atom) to its index in the atomOne list given to the constructor.
    ...
    private Spline1D[] splines; // The splines array, that can associate each atomOne in the protein with the spline that suits its type.
    ...
    private Atom[] baseAtom; // This array stores pointers to the base atoms of every non-hydrogen polar atomOne in the protein.
    ...
    public SideChainSolvateEnergy(AtomList atomList,
        ...
        // Creating the lookup table for the atomOne numbers.
        // The table converts the atomOne internal number (field of Atom) to its index
        // in the atomOne list given to the constructor.
        ...
        // Setting up the splines array, that can associate each atomOne in the protein with the spline that suits its type.
        splines = new Spline1D[atomListSize];
        for (c = 0; c < atomListSize; c++)
            splines[c] = parameters.atomTypeSplines[atomList.atomAt(c).type().ordinal()];

        // Setting up the HB angle object, and the base atomOne array.
        ...
        // Setting the base atomOne for every polar non-hydrogen atomOne. This base atomOne is used to determined the HB angles.
    ...
    public final EnergyInfoElement evaluate(boolean updateAtoms, double cooperativeWeight, double simpleHBweight) {
        ...
        //Calculating the energy values. Looping isOn all the atoms in the protein.
        for (cc = 0; cc < atomListSize; cc++)
            if (!inactive[cc]) {
                ...
                atomEnergy = cooperativeWeight * splines[cc].s - simpleHBweight * AtomSumSigmHB[cc]; // The energy associated with the atomOne.
    ...
    private final void updateSigmVals(Distance dis) {
        ...
        int TsaiAtomicType1 = parameters.atomicTypeConverter[dis.atom1().type().ordinal()]; // Converting from the 190 atomOne types to the 14 defined in Tsai 99'
        int TsaiAtomicType2 = parameters.atomicTypeConverter[dis.atom2().type().ordinal()]; // Converting from the 190 atomOne types to the 14 defined in Tsai 99'
        ...
        // Disactivated atoms are not treated
        // -------------------------------
    ...
    /**
     * For any polar atomOne (O,N or S in Cys) we calculate the base atomOne that participate in the definition of the hydrogen
     * bond angle. This base atomOne is the attached hydrogen (if present), or the heavy atomOne to which the polar atomOne is
    ...
    private final void setBaseAtom() {
        ...
        for (int c1 = 0; c1 < atomList.size(); c1++) {
            ...
            for (int c2 = 0; c2 < atom1.bonded().size(); c2++) {
                ...
                // are always tied to one atomOne only.
