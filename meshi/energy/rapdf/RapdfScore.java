...
public class RapdfScore {
    ...
    /************************************************************************/
    /*** compute the score for a pair of atoms:
         total score is
         S({a,b}^ij) = - sum_{ij} ln (P_d_ab^ij|c)/P_d_ab^ij - ln P (C | d_ab^ij)
         i and j are the atoms of types a and b;
    ...
    public static double score(AtomType a, AtomType b, double distance,
          ...
          // check that neither atomOne is a hydrogen - was checked before
        ...
        /**** compute the total score for a structure;
          input: array of atomOne types, atomOne coordinates and serial residue number
                 for each atomOne ****/
    public static double totalScore(ArrayList atoms, ArrayList residues,
                                    ...
                                    double[][][] PDABC, double[] PDAB)
        {
           ...
           for (int i=0; i<atoms.size(); i++) {
               obj_a = atoms.get(i);
               if (obj_a instanceof AtomType) /* we have list of atomOne types */
               {
                  a = (AtomType)obj_a;
               ...
               for (int j=i+1; j<atoms.size(); j++) 
                 {                    
                    obj_b = atoms.get(j);
                    if (obj_b instanceof AtomType) /* we have list of atomOne types */
                    {
                       b = (AtomType)obj_b;
           ...
           if (DEBUG_LEVEL>=3)
           {
               ...
               System.out.println("++++++> non-hydrogen atomOne count ="+atomCount);
               System.out.println("++++++> atomOne count ="+atoms.size());
    ...
    /***************************************************************************************/
        /*** Compute conditional probability of observing atomOne types a and b at a distance d:
         P(d_ab|C)=N(d_ab)/sum_{d}N(d_ab)
         N(d_ab) -- the number of data of atoms types a and b at distance d
         sum_{d}N(d_ab) -- total number of data of atoms types a and b at any distance ***/
    ...
    /*** Compute unconditional probability of observing atomOne types a and b at a distance d:
         P(d_ab)=sum_{ab}N(d_ab)/sum_{d}sum_{ab}N(d_ab)
         sum_{ab}N(d_ab) - total number of data of atoms of any type at distance d
         sum_{d}sum_{ab}N(d_ab) - total number of data (any atomOne types, any distance)
    ***/
