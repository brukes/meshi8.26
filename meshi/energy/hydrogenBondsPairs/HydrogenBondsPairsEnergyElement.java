...
public class HydrogenBondsPairsEnergyElement extends NonBondedEnergyElement {
    ...
    *   where P(D) = the prior  prob to have 2 HB when picking 2 pairs of H-O atoms.Itdependes reset the number of residues that are in secondary strucutre segments
    *  yet, in thus version it will be a constant number
    *  given a specific pattern <x1,x2,x3,x4,x5,x6> P(T) is the number that this pattern was counted when we looked at two pairs of H-O atoms, in whicn at least
    ...
    * */
    ...
    private double PD_Beta=0.005, PD_Helix=0.0007; //P(D) for beta or helix
        private double PT_Beta_Background = 3987207, PT_Helix_Background = 57265603; //  the number of all patterns when we looked at two pairs of H-O atoms, in which at least  one pair is bounded
    private double PT_Beta_spesificPattern, PT_Helix_spesificPattern; //  given a specific pattern <x1,x2,x3,x4,x5,x6> P(T) is the number that this pattern was counted when we looked at two pairs of H-O atoms, in whicn at least one pair is bounded
    private double PT_givenD_Beta_Background = 54891,PT_givenD_Helix_Background=128659; //  the number of all patterns when we looked at two pairs of H-O atoms, in which both pairs are bounded
    private double PT_givenD_Beta_spesificPattern,PT_givenD_Helix_spesificPattern; //  given a specific pattern <x1,x2,x3,x4,x5,x6> P(T) is the number that this pattern was counted when we looked at two pairs of H-O atoms, in whicn both  pairs are bounded
    ...
    public HydrogenBondsPairsEnergyElement(
        ...
        if(specialDisArray != null)
            System.out.println("********************* Warning: check that setResidue treat this case!!!");
    }
    ...
    public void set(Object obj){//obj should be PairOfHydrogenBondsElements
      ...
      //  else {
                ...
                if(pair.getPairValue() == -1000 | pair.getPairBackgroundValue() == -1000){
                            if(pair.isHelixPair() ) {
                                    ...
                                    factor = -1*log_PD_givenT_Helix*HELIX_FACTOR; //HELIX_FACTOR was added by Chen, Sept. 2011
                                      ...
                                      pair.setPairValue(parameterValue );
                                      pair.setPairBackgroundValue(backgroundParameterValue);
                                    //Most pv and bpv are nonExistValue (-5)
                                    //System.out.println(oAtom1.residueNumber() +" "+hAtom1.residueNumber()+" "+oAtom2.residueNumber() +" "+hAtom2.residueNumber() +" factor: " + factor +" pv: "+parameterValue+" bpv: "+backgroundParameterValue +" punish: "+punish);
        ...
        //System.out.println(oAtom1.residueNumber() +" "+hAtom1.residueNumber()+" "+oAtom2.residueNumber() +" "+hAtom2.residueNumber() +" factor: " + factor);
        ...
        if (pair.seqDistance1() == -4 & //parameters.firstHBSeqGap == -4 &
                ...
                factor = 1;  //This is a helix !       changed reset 27/11/06 21:00
                     ...
                     //if(Math.abs((pair. hAtom1.residueNumber() - pair. hAtom2.residueNumber())) > 5)                  //TODO add a spacial punish to this case, if needed (if results indicates that there are more then expected o atoms with more the one hbonds
                           // System.out.println("HBPEnergyElement: same o atomOne: "+pair);
    ...
    */
    public double evaluate() {
      ...
      //      x = 1;
        ...
        if(distanceAttributes1 != HOelement1.getAttribute(HB_DistanceAttribute.key))
            System.out.println("this is wird");
        if (energy*weight>100) {
            System.out.println("h1: " + hAtom1.residueNumber() + " o1: " + oAtom1.residueNumber() + " h2: " + hAtom2.residueNumber() + " o2: " + oAtom2.residueNumber() + " energy*weight: " + energy*weight);
        }
        return energy * weight;
    }

    public double updateEnergy() {
        //Most factors here are 10
        //System.out.println(oAtom1.residueNumber() +" "+hAtom1.residueNumber()+" "+oAtom2.residueNumber() +" "+hAtom2.residueNumber() +" factor: " + factor);
        ...
        //Almost all factors here are 10
        return energy;
    }
