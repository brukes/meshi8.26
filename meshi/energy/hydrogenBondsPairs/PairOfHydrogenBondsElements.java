...
public class PairOfHydrogenBondsElements {
    ...
    /*
     * The hydrogen residue number in HOelement1 <= the hydrogen residue number in HOelement2
     */
    protected Distance HOelement1;
	protected Distance HOelement2;	
	
    /*
     * seqDistance means the sequential gap between 2 atoms in the list of atoms.
     */
    ...
    /*
     * we determined that 2 HBpairs are close by using MAX_SEQ_PAIRS_DISTANCE.
     */
    public final int MAX_SEQ_PAIRS_DISTANCE = 5;
    /*
     * determined by criteria that can be found in the method:
    ...
    private static final int HB_PAIR_CAPACITY = 4; //should be 6 if we want also Nitrogen
   ...
   private SsElement ssElement;
    ...
    private boolean helixPair,betaPair, oneHelix = false, oneBetta = false, goodPair=false;

    //Ofer's addition. Whether or not the pair is good according to our criteria, for now it is that the HBonds
    //are real, meaning that their distance is under 2.5 angstroms.

    public boolean isGoodPair() {return goodPair;}
    ...
    PairOfHydrogenBondsElements(PairOfHydrogenBondsElements pair) {
        ...
        if(lookAtThisPair){
            ...
            throw new RuntimeException("This is weird: copy of bad pair "+pair);
        this.goodPair = pair.goodPair;
    }
    ...
    public  void set (Distance _HOelement1,Distance _HOelement2){
       ...
       //boolean secondaryStructure; //if both the residues are predicted to be with the same SS and in good positions (in helix) or true if we are in coil...
...
//
        ...
        if((_HOelement1.distance <= 2.5) && (_HOelement2.distance <= 2.5)){
            goodPair=true;
        }
        else{
            goodPair=false;
    ...
    public Atom hAtom(Distance atomPair){
        ...
        else throw new RuntimeException("This pair does not have hydrogen "+atomPair);
    }
    ...
    public Atom oAtom(Distance atomPair){
        ...
        if (atom1Attribute.isO)
            ...
            return atomPair.atom2();
        else throw new RuntimeException("This pair does not have oxygen "+atomPair);
    }
	    
    public static boolean helixPair(Distance atomPair){
        SecondaryStructure atom1SS = atomPair.atom1().residue().secondaryStructure();
        SecondaryStructure atom2SS = atomPair.atom2().residue().secondaryStructure();
    ...
    public static boolean sheetPair(Distance atomPair){
        SecondaryStructure atom1SS = atomPair.atom1().residue().secondaryStructure();
        SecondaryStructure atom2SS = atomPair.atom1().residue().secondaryStructure();
    ...
    public String toString(){
        return "PairOfHydrogenBondsElements: "+ssElement+"  "+hAtom1.residueNumber()+" "+oAtom1.residueNumber()+
            " "+hAtom2.residueNumber()+" "+oAtom2.residueNumber()+ " goodPair: "+goodPair + " "+HOelement1.mode()+" "+HOelement2.mode()+
