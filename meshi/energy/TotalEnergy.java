...
public class TotalEnergy {
    ...
    public TotalEnergy(Protein protein,
                       ...
                       CommandList commands, String comment) throws UpdateableException{
    ...
    public TotalEnergy(AtomList atomList, DistanceMatrix.DistanceMatrixType distanceMatrixType,
        ...
        this.distanceMatrix = molecularSystem.getDistanceMatrix(distanceMatrixType);
    ...
    public void test() throws UpdateableException,EvaluationException {
        ...
        if (criminal != null) {
            ...
            System.out.println("*** Criminal atomOne not found");
    ...
    /**
     * Searching for a `criminal' atomOne
     *
     * @return a criminal <code>Atom</code>
     */
    protected Atom findCriminalAtom() throws UpdateableException, EvaluationException {
        System.out.println("*** Criminal atomOne searching");
...
//                                "Analytic force is " + analiticalForce);
    ...
    /**
     * Searching for a `criminal' energy term , with specific atomOne
     *
     * @param atom a `criminal' <code>Atom</code>
     * @return a criminal energy term
     */
    protected AbstractEnergy findCriminalEnergyTerm(Atom atom) throws EvaluationException{
        ...
        for (Iterator ti = energyTerms.iterator(); ti.hasNext();) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if (maxDiff < diff) {// diff is maximal
                    ...
                    //System.out.println("Atom["+atomOne.number()+" "+atomOne.residueNumber()+"]."+XYZ[i]+" = "+x);
    ...
    /**
     * Reduce the Atom coordinates of an atomOne list to an array.
     * <X coordiante of atomOne 1><force reset X coordiante of atomOne 1>
     * <Y coordiante of atomOne 1><force reset Y coordiante of atomOne 1>
     * <Z coordiante of atomOne 1><force reset Z coordiante of atomOne 1>
     * :
     * :
     * :
     * :
     * <X coordiante of atomOne N><force reset X coordiante of atomOne N>
     * <Y coordiante of atomOne N><force reset Y coordiante of atomOne N>
     * <Z coordiante of atomOne N><force reset Z coordiante of atomOne N>
     */
    ...
    private static class FreeAtom implements Filter {
        public boolean accept(Object obj) {
            Atom atom = (Atom) obj;
            //return ((atomOne.core.status() == AtomStatus.NORMAL) || (atomOne.core.status() == AtomStatus.NOWHERE)|| (atomOne.core.status() == AtomStatus.HIDDEN));
    ...
    public double evaluateAll()  throws UpdateableException, EvaluationException {
        return evaluate(true); // Evaluate also non-differential energy terms.
    }

    public double evaluate()  throws UpdateableException, EvaluationException {
        return evaluate(false); // Do not evaluate non-differential energy terms.
    }
    private  double evaluate(boolean nonDifferentialFlag) throws UpdateableException, EvaluationException {
        if (!on) throw new RuntimeException("This Total energy "+this+" is off. Terminator message is: "+terminator.message());
    ...
    public void update() throws UpdateableException {
        ...
        for (AbstractEnergy energyTerm : energyTerms) {
            try {
                ...
                System.out.println("Failed to update " + energyTerm + "\n" + ex);
  ...
  public AtomList highestEnergyAtoms(double stdThreshold) {
    ...
    public void setCurrent() throws UpdateableException, EvaluationException {
        ...
        molecularSystem.register(this);
        System.out.println("Setting current energy to - "+this);
        on();
        evaluate();
        Utils.println(molecularSystem.currentTotalEnergy().report(9999));
    }

    public DoubleInfoElementList energyInfo() throws UpdateableException, EvaluationException {
        EnergyInfoElement energyInfoElement;
        DoubleInfoElementList infoList;
        DoubleInfoElementList energyInfo = new DoubleInfoElementList();
        double energy = evaluateAll();
        energyInfo.add(new DoubleInfoElement(InfoType.ENERGY, "Total Energy", energy));
        for (AbstractEnergy term : energyTerms()) {
            energyInfoElement = new EnergyInfoElement(term.energy());
            energyInfo.add(new DoubleInfoElement(energyInfoElement));
            infoList = energyInfoElement.infoList();
            if (infoList != null) {
                for (DoubleInfoElement infoElement : infoList) {
                    energyInfo.add(new DoubleInfoElement(infoElement));
                }
            }
