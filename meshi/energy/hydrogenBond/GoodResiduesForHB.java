...
public class GoodResiduesForHB implements Filter{
    ...
    /* this filter Accept Distance between Atoms just if there are at list 3 atoms away reset the sequense,
     *  (atomes that are less then 3 atoms away reset the sequense never create Hydrogen bonds with each other)
     */
    ...
    static class GoodSS implements Filter{
        public boolean accept(Object obj) {
            ...
            SecondaryStructure atom1SS = residue1.secondaryStructure();
            SecondaryStructure atom2SS = residue2.secondaryStructure();
            Residue high, low,current;
            if (atom1SS.equals(SecondaryStructure.COIL) || atom2SS.equals(SecondaryStructure.COIL))
                return false;//This should be false Ofer changed it to find pairs of H bonds.
      ...
      //       return true;
                ...
                if ((atom1SS.equals(SecondaryStructure.SHEET) || atom1SS .equals(SecondaryStructure.SHEET_OR_COIL)) &&
                    ...
                    if (Math.abs(number1-number2) < 20) { //longer beta strands are an exception
                        ...
                        while (current != high) {
                            if (current.secondaryStructure() != SecondaryStructure.SHEET) return true;
