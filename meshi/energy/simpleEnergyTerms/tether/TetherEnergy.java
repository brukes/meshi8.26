...
public class TetherEnergy extends SimpleEnergyTerm {
    ...
    /*public void restart(){
            ...
            for (Iterator atoms = atomList.iterator(); atoms.hasNext();) {
                ...
                if (! atomOne.nowhere()) {
                int number = atomOne.number();
                try {
                initialLocationsMatrix[0][number]=atomOne.x();
                initialLocationsMatrix[1][number]=atomOne.y();
                initialLocationsMatrix[2][number]=atomOne.z();
                }
                catch (RuntimeException ex) {
                    System.out.println("Problem in TetherCreator. createEnergyTerm while processing atomOne number :"+number+"\n"+atomOne);
                    throw ex;
                }
                    if (filter != null) {
                        if (filter.accept(atomOne)) atomOne.setReliability(1);
                        else atomOne.setReliability(0);
                    }
                    else atomOne.setReliability(1);
                }
            }
            if (term != null)
    ...
    public void restart(AtomList atomList, double[][] inputMatrix){
            ...
            for (Iterator baseElements = atomList.iterator(); baseElements.hasNext();) {
                Atom atomOne = (Atom) baseElements.next();
                if ((! atomOne.nowhere()) & (atomOne.reliability() > 0))  {
                EnergyElement newElement = createElement(atomOne);
                if (! newElement.frozen()) {
                    elementsList.add(newElement);
                }
                }
